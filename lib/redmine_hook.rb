class RedmineHook  < Redmine::Hook::ViewListener

    def view_layouts_base_html_head(context = {})
        stylesheet_link_tag('that', :plugin => 'redmine_email_log') +
        javascript_include_tag('that', :plugin => 'redmine_email_log')
    end

    def view_layouts_base_content(context = {})
        if User.current.admin? && Setting.plugin_redmine_email_log['contextual_email_logs'] && context[:request].format == 'text/html'
